...
int priority_calculate() {
	...
	for (int i = 0; i < (N / 2); i++) {
		...
		else v[0][i + 1] = calculate(v[0][i], v[1][i], v[0][i + 1]);
	}
	w[0].push_back(v[0][N / 2]);
	int ans = w[0][0];
	for (int i = 0; i < w[1].size(); i++) ans = calculate(ans, w[1][i], w[0][i + 1]);
	v = backup;
	return ans;
}
...
int backTracking(int idx) {
	...
	//printState();
	//cout << "ans : " << ans << endl;
	return max(result, priority_calculate());
}
